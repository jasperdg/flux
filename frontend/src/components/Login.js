...
class Login extends Component {
	...
	render() {
		...
		const message = `NODE_ENV=local near create_account {newAccountId} --masterAccount {masterAccountId} --publicKey ${new URL(currentUrl).searchParams.get('public_key')} --initialAmount 10000000000000000000`;
		...
		return(
			<>
				...
				<button type="button" onClick={this.done}>done</button>
			</>
		);
	}
}
